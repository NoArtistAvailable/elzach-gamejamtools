...
public static class AudioSave
{
    ...
    public static void Save(string filepath, AudioClip clip, bool trim = false, float trimLevel = 0f)
    {
        ...
        {
            var wav = GetWav(clip, out var length, trim, trimLevel);
            writer.Write(wav, 0, (int)length);
        }
    }

    public static byte[] GetWav(AudioClip clip, out uint length, bool trim=false, float trimLevel=0f)
    {
        var data = ConvertAndWrite(clip, out length, out var samples, trim, trimLevel);
    ...
    private static byte[] ConvertAndWrite(AudioClip clip, out uint length, out uint samplesAfterTrimming, bool trim, float trimLevel)
    {
        ...
        if (trim)
        {
            ...
            {
                if (Mathf.Abs((short)(samples[i] * RescaleFactor)) <= trimLevel)
                {
                    continue;
                }
                //Debug.Log("Startsample: " + i + " : " + Mathf.Abs((short)(samples[i] * RescaleFactor)));
                start = i;
                break;
            }
